	...
	for (const source of sources) {
		if (Array.isArray(source)) {
			...
			for (let [key, value] of Object.entries(source)) {
				...
				returnValue = _extends({}, returnValue, { [key]: value });
			}
		}
	}
...
const timeout = (promise, ms) => Promise.race([promise, (async () => {
	await delay(ms);
...
class Ky {
	constructor(input, _ref) {
		let { timeout = 10000, hooks = { beforeRequest: [] }, throwHttpErrors = true, json } = _ref,
		    otherOptions = _objectWithoutProperties(_ref, ['timeout', 'hooks', 'throwHttpErrors', 'json']);
